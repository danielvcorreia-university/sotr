...
int main(int argc, char *argv[])
{
	...
	/* Process input args */
	if(argc != 3) {
	  printf("Usage: %s PROCNAME PRIORITY, where PROCNAME is a string and PRIORITY is the thread priority\n\r", argv[0]);
	  return -1; 
	}

	if(atoi(argv[2]) < 1 || atoi(argv[2]) > 99) { 
	  printf("Usage: %s PROCNAME PRIORITY, parameter PRIORITY must be between 1 and 99 (inclusive)", argv[0]);
	  return -1;
	}
	...
	/* Defining the priority of the lauch thread */
	pthread_attr_init(&attr);
	/* Use the scheduling policy specified in attr for the thread. (PTHREAD_EXPLICIT_SCHED) */
	pthread_attr_setinheritsched(&attr,
								PTHREAD_EXPLICIT_SCHED);
	/* Defining the schedualing policy to SCHED_FIFO: First in-first out scheduling */
	pthread_attr_setschedpolicy(&attr, SCHED_FIFO);
	parm.sched_priority = atoi(argv[2]);
	printf("Priority of thread : %s\n\r", argv[2]);
