...
#define TASK_A_PRIO 75 	// RT priority [0..99]
...
RT_SEM sem1;   // Semaphore descriptor
...
void task_code(void *args) {
	...
	for(;;) {
		...
		if(strcmp(curtaskinfo.name,"2")==0 ||  strcmp(curtaskinfo.name,"3")==0){
			err = rt_sem_p(&sem1,TM_INFINITE);
        	if(err) {
		...
		if(strcmp(curtaskinfo.name,"1")==0 ||  strcmp(curtaskinfo.name,"2")==0){
			errv = rt_sem_v(&sem1);
        	if(errv) {
